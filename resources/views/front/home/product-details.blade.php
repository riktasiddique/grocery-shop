        ...
        <div class="col-md-6">
            <div class="card mt-5">
                <div class="card-body text-center border  border-success">
                    ...
                    <div class="price mt-2"><p>Total Price: <span id="productPriceId{{$product->id}}">{{$product->price}}</span> Tk</p></div>
                    <input type="hidden" value="{{$product->price}}" id="unitPrice{{$product->id}}">
                    <form action="{{route('home-add-cart.store', $product->id)}}" method="post">
